...
export default function SingleProjectPage() {
  ...
  const fetchProject = useCallback(async () => {
    ...
    try {
      const res = await axiosInstance.get<ProjectApiResponse>(`/projects/${id}`);
      setProject(res.data?.data || null);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (error: any) {
      console.error("Single Project fetch error:", error.response?.data || error.message);
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  const allImages = [project.mainImage, ...(project.galleryImages || [])].filter(Boolean) as string[];
  ...
  return (
    <div className="min-h-screen mt-24 bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 relative z-10">
        ...
        >
          <Link href="/projects" className="flex items-center gap-2 text-gray-600 hover:text-[#164C36] transition-colors duration-300">
            <ArrowLeft size={20} />
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 py-6">
              <div className="text-center p-4 bg-green-50 rounded-2xl">
                <div className="text-2xl font-black text-[#164C36] mb-1">100+</div>
        ...
        >
          ...
          <p className="text-white/90 text-lg mb-8 max-w-2xl mx-auto">
            Contact our sales team today to get detailed information, pricing, and availability for this exceptional development.
          </p>
