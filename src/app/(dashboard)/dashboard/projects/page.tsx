...
const ProjectPage = () => {
  ...
  // ✅ Amenities separate input state
  const [amenitiesInput, setAmenitiesInput] = useState(""); // raw input
  const [amenities, setAmenities] = useState<string[]>([]); // array for edit
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      console.error("[DEBUG] Fetch projects error:", err);
  ...
  const openEditDrawer = (project: Project) => {
    ...
    setAmenitiesInput((project.amenities || []).join(", ")); // input field
    setIsDrawerOpen(true);
  };
  ...
  const handleGalleryChange = (files: FileList | null) => {
    ...
    setGalleryFiles((prev) => [...prev, ...newFiles]); // append new files
  ...
  const handleSubmit = async () => {
    ...
    // ✅ Convert amenitiesInput to array
    ...
    try {
      if (editingProject) {
        await axiosInstance.patch(`/projects/${editingProject._id}`, formData, {
        ...
        >
          <div>
            ...
            <p className="text-white/90 mt-1">
              Manage and explore projects in a modern dashboard
            </p>
          </div>
          <button
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y">
              {projects.map((project, i) => {
                const cityName =
                  cities.find((c) => c._id === project.cityId)?.name || "—";
                return (
                  <tr
                    key={project._id}
                  ...
                  >
                    ...
                    <td className="p-4 text-sm text-gray-600">{cityName}</td>
